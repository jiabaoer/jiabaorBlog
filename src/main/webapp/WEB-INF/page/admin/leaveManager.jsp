...
<head>
    ...
    <script>
        layui.use('jquery', function () {
            layui.$(function () {
                layui.$(".layui-nav-tree li").eq(6).addClass("layui-nav-itemed");
            });
        });
    </script>
    <style>
                        ...
                        <div id="LAY_preview">
                            <div class="layui-container">
                                <table class="layui-table">
                                    ...
                                    <tr>
                                        <th colspan="6">
                                            <button class="layui-btn layui-btn-danger" id="leave_delete_all_btn"
                                    ...
                                    <tr>
                                        ...
                                        <th>ID</th>
                                        <th>用户名称</th>
                                        <th colspan="2">留言内容</th>
                                        <th>留言日期</th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                    <c:forEach items="${pageInfo.list}" var="lea">
                                        <tr>
                                            <td>
                                            ...
                                            <td>${lea.id}</td>
                                            <td>${lea.user.nickname}</td>
                                            <td colspan="2">${lea.leaveText}</td>
                                            <td>${lea.leaveTime}</td>
                                        </tr>
                                    </c:forEach>
                                    <tr align="center">
                                        <td colspan="3">
                                            <a href="/admin/leaveManager?pageNumber=${pageInfo.pageNumber-1 }&pageSize=${pageInfo.pageSize}"
...
<script>
    layui.use(['jquery', 'layer'], function () {
        ...
        //点击全部删除，就批量删除
        $("#leave_delete_all_btn").click(function () {
            var delPicture_ids = "";
            ...
            layer.confirm("确认删除【" + delPicture_ids + "】吗？", function () {
                //发送ajax请求删除
                $.ajax({
                    url: "/admin/delLeave_ids/" + delPicture_ids,
                    type: "POST",
